...
class TmTaskInfoForm(ModelForm):
    class Meta:
        ...
        widgets = {
            "task_title": Textarea(
                attrs={"rows": 2, "cols": 15, "class": "title-area"}
            ),
            "task_description": Textarea(
                attrs={"rows": 3, "cols": 30, "class": "description-area"}
            ),
            "start_date": DateInput(attrs={"type": "date", "class": "date-time"}),
            "created_by": Select(),
            "modified_by": Select(),
            "end_date": DateInput(attrs={"type": "date", "class": "date-time"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["task_title"].required = True
